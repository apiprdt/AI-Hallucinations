...
Respond with ONLY a number between 0 and 1. Nothing else."""
    ...
    def run_eval(self, task: str):
        ...
        results_file = f"data/processed/results_Llama3.3-70B_{task}.json"
        ...
        print(f"Running Llama3.3-70B-2-9b-it on {task.upper()} (N={total_mols})...")
        ...
        try:
            ...
            print(f"[{task.upper()}] Llama3.3-70B-2-9B AUC | C0: {auc_c0:.3f} | C3: {auc_c3:.3f}")
        except ValueError:
            print(f"Could not calculate AUC for {task}")

if __name__ == "__main__":
    runner = Llama3.3-70BRunner()
