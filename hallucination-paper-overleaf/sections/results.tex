...
Table~\ref{tab:results} summarizes ROC-AUC scores, bootstrap 95\% confidence intervals, and statistical comparisons for all nine conditions on both datasets.
...
Free hallucination (C3) improved ROC-AUC to 0.594 [0.519, 0.664]. Among taxonomy-specific conditions, Structural Phantom hallucination (C4a) yielded the highest observed ROC-AUC of 0.626 [0.548, 0.698], followed by Mechanism Fabrication (C4c, 0.612) and Property Inversion (C4b, 0.561). 
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/figure4_bootstrap_stability.png}
    \caption{Distribution of bootstrapped ROC-AUC scores (1,000 resamples) on the BBBP test set. The dashed line represents the median baseline (C0) performance. Structural Phantom (C4a) exhibits a stable distributional shift above baseline.}
    \label{fig:bootstrap}
\end{figure}
...
\begin{figure}[t]
    ...
    \caption{Prediction probability distributions on BACE under baseline (C0) and free hallucination (C3) conditions. Under C3, predictions collapse to a narrow distribution centered near $p \approx 0.17$, eliminating discriminative capacity.}
    \label{fig:negativity}
\end{figure}

\subsection{Hallucination Taxonomy Distribution}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/figure2_taxonomy_distribution.png}
    \caption{Distribution of hallucination types among naturally generated (C3) descriptions on BBBP, classified by the rule-based taxonomy. CC dominates at $\sim$80\%, while SP accounts for $\sim$2.4\%.}
    \label{fig:taxonomy}
\end{figure}
...
\begin{figure}[t]
    ...
    \caption{ROC-AUC scores across nine experimental conditions on BBBP. Dashed line indicates baseline (C0) performance. SP hallucination (C4a) yields the highest observed ROC-AUC; the shuffled control (C5) aligns with baseline.}
