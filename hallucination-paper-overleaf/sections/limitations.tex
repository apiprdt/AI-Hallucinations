...
\subsubsection*{Prompt divergence across experiments.} The prediction and hallucination generation prompts used for the extended benchmarks (Tox21, ESOL) were adapted from, but not identical to, the primary ablation prompts (BBBP, BACE). While the prompts serve the same functional role, the exact phrasing differs (see Appendix for full prompts). This constitutes a methodological confound that limits the interpretability of direct cross-task comparisons; observed differences in semantic sensitivity between the primary and extended benchmarks may partially reflect prompt-specific effects rather than purely task-dependent phenomena.

\subsubsection*{Methodological confounds.} The BACE Mechanism Fabrication (C4c) condition directly prompts the LLM to hypothesize biological targets. Because BACE-1 is the implied target of the BACE dataset, the MF text may inadvertently contain information about BACE-1 itself. Crucially, we note a limitation in our post-hoc verification: the heuristic classifier's keyword list for MF detection (see Appendix A) inadvertently omitted ``BACE-1'', rendering the automated verification blind to this specific confound. Paradoxically, however, C4c produced directional AUC degradation on BACE (0.566, $p_{\text{BH}} = 0.083$), suggesting that even semantically relevant target information fails to counteract the distributional compression effect when embedded within a fabrication prompt. Although we validated the C5 control across multiple permutation seeds ($0.49 \pm 0.02$), a larger number of permutations with full-dataset evaluation would further strengthen this control.
